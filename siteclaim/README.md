...
against the Construction Industry Security of Payment Ordinance with a
deterministic Rules Engine, drafts a compliant payment claim grounded in CIC
...
> ⚠️ Statutory parameters in `backend/rules_engine/cisop_config.py` are
> **unverified placeholders from secondary research**. Validate every value with
> a quantity surveyor or construction lawyer before relying on output. SiteClaim
